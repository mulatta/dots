---
name: workaround
description: Temporary workaround to keep progress while waiting for a real fix.
defaults:
  priority: 3
  labels:
    - type:workaround
...
schema:
  ...
  properties:
    ...
    checklist:
      ...
      maxItems: 5
      description:
        Milestones like apply workaround, verify, track real fix, remove
        later.
    notes:
      type: array
      items:
    ...
    proof:
      ...
      items:
        type: string
      maxItems: 3
      description: Build, test, or log showing workaround or removal is safe.
    sources:
      type: array
      items:
        $ref: "#/$defs/Source"
...
attachment_expectations:
  - Build, test, or log showing workaround or removal is safe.
---

# workaround

Temporary workaround to keep progress while waiting for a real fix.
