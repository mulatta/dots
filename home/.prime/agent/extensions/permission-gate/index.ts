...
export default function permissionGate(pi: ExtensionAPI) {
	...
	function updateStatus(ctx: ExtensionContext): void {
		if (!ctx.hasUI) return;
		ctx.ui.setStatus("gate", promptsEnabled ? "\uf132 gate" : "\uf132 block");
	}
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		// A daemon UI context does not prove that an approval-capable client is
		// attached. Keep project configuration under headless trust rules; the
		// primitive confirmation below still permits an attached client to approve.
		await reloadRules(ctx.cwd, warn, true);
		updateStatus(ctx);
	});
