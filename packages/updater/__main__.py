...
def fix_placeholder_hash(pkg: Package, flake_root: Path) -> bool:
    """Fix placeholder npmDepsHash by building the package and extracting the correct hash."""
    target_file = None
    for filename in ("default.nix", "srcs.json"):
        filepath = pkg.path / filename
        if filepath.exists() and PLACEHOLDER_HASH in filepath.read_text():
    ...
    print(f"  Fixing npmDepsHash in {target_file.name}...")

    result = run_cmd(
        ["nix", "build", f".#packages.x86_64-linux.{pkg.name}", "--no-link"],
        cwd=flake_root,
        check=False,
    )
    ...
    print(f"  Got hash: {correct_hash}")

    content = target_file.read_text()
    target_file.write_text(content.replace(PLACEHOLDER_HASH, correct_hash))
    return True


def update_package(pkg: Package, flake_root: Path, dry_run: bool = False) -> bool:
    ...
    if pkg.method == "nix-update":
        ...
        if not run_custom_update(pkg, dry_run):
            return False
        if not dry_run:
            return fix_placeholder_hash(pkg, flake_root)
        return True
    else:
...
def _run_update_and_create_pr(
    ...
    if pkg.method == "nix-update":
        ...
        if success:
            success = fix_placeholder_hash(worktree_pkg, worktree_path)
    else:
        success = False
    ...
    # Ensure git identity is configured (worktrees may not inherit CI config)
    for key, val in [
        ("user.name", "github-actions[bot]"),
        ("user.email", "41898282+github-actions[bot]@users.noreply.github.com"),
    ]:
        check = run_cmd(["git", "config", key], cwd=worktree_path, check=False)
        if check.returncode != 0:
            run_cmd(["git", "config", key, val], cwd=worktree_path)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Update third-party packages in ./packages"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
