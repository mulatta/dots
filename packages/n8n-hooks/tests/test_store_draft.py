...
def test_attachment(tmp_path: Path) -> None:
    ...
    assert base64.b64decode(att["data"]) == b"%PDF-fake"
...
def test_default_html_body_escapes_plain_text() -> None:
    args = _make_args(body_plain="<hello> & goodbye")
    payload = build_payload(args)

    assert payload["body_html"] == "<pre>&lt;hello&gt; &amp; goodbye</pre>"
...
def test_end_to_end(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    config = {
        "store-draft": HookConfig(
            url=f"http://127.0.0.1:{port}/webhook/mail-draft-store",
            token="test-secret",
        ),
    }
    args = _make_args()
