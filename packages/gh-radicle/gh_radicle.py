...
def setup_github_secrets(
    ...
    Note: This sets up Radicle-specific secrets only.
    You must separately set up GitHub App secrets (APP_ID and APP_PRIVATE_KEY)
    for the workflow to function. These are shared across repositories and should
    be set once at the organization/user level.
    """
    env_name = "radicle"
...
def setup_mirror(config: MirrorConfig) -> int:
    ...
    print("\n" + "=" * 60)
    print("IMPORTANT: GitHub App Setup Required")
    print("=" * 60)
    print("\nThe workflow requires a GitHub App for authentication.")
    print("If you haven't set up a GitHub App yet, follow these steps:")
    print("\n1. Create a GitHub App:")
    print("   https://github.com/settings/apps/new")
    print("   OR use: create-gh-app radicle-sync \\")
    print("     --permission contents write \\")
    print("     --permission pull_requests write")
    print("\n2. After creating the app, get the App ID and generate a private key")
    print("\n3. Set the following secrets (one-time setup):")
    print(f"   gh secret set APP_ID --repo {config.github_repo}")
    print(f"   gh secret set APP_PRIVATE_KEY --repo {config.github_repo}")
    print("\n4. Install the app on your repository:")
    print("   https://github.com/settings/apps/radicle-sync/installations")

    print("\nNext steps:")
    print("  1. Set up GitHub App secrets (see above)")
    print("  2. Commit and push the workflow file")
    print("  3. The mirror will sync on every push to main")
