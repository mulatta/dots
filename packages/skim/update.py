#!/usr/bin/env python3
"""Update skim from GitHub releases."""
...
GITHUB_API = "https://api.github.com/repos/skim-rs/skim/releases/latest"
...
def update_nix_file(nix_file: Path, version: str, sri_hash: str) -> None:
    ...
    text = re.sub(
        r'(owner = "skim-rs";\s+repo = "skim";\s+tag = "v\$\{version\}";\s+hash = ")[^"]+"',
        rf'\g<1>{sri_hash}"',
        text,
    )
...
def main():
    ...
    sri_hash = prefetch_github("skim-rs", "skim", f"v{version}")
