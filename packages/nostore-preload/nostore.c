 ...
 *
 * The open-family wrappers only consume variadic `mode` when flags require
 * it; reading an absent variadic argument is undefined behavior.
 */
...
static void hint(void) {
  ...
  if (atomic_exchange_explicit(&done, 1, memory_order_relaxed))
    return;
...
static int open_requires_mode(int flags) {
  if (flags & O_CREAT)
    return 1;
#ifdef O_TMPFILE
...
#ifdef __APPLE__
typedef int nostore_mode_arg_t;
#else
typedef mode_t nostore_mode_arg_t;
#endif
...
#define FORWARD_OPEN(real, path, flags)                                        \
  do {                                                                         \
    if (open_requires_mode(flags)) {                                           \
      va_list ap;                                                              \
      va_start(ap, flags);                                                     \
      nostore_mode_arg_t mode = va_arg(ap, nostore_mode_arg_t);                \
      va_end(ap);                                                              \
...
#define FORWARD_OPENAT(real, dirfd, path, flags)                               \
  do {                                                                         \
    if (open_requires_mode(flags)) {                                           \
      va_list ap;                                                              \
      va_start(ap, flags);                                                     \
...
WRAPPER(int, openat)(int dirfd, const char *path, int flags, ...) {
  ...
  FORWARD_OPENAT(real, dirfd, path, flags);
}
WRAPPER_DEF(openat)
