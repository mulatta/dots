# Register ~/bin/herdr-open-file as the system handler for text/source files,
# so clicking a file:// hyperlink in ghostty (or any terminal) opens helix
# in the active Herdr pane instead of TextEdit/gedit/whatever.
#
...
let
  ...
  opener = "${config.home.homeDirectory}/bin/herdr-open-file";
...
lib.mkMerge [
  (lib.mkIf pkgs.stdenv.isLinux {
    ...
    xdg.mimeApps = {
      enable = true;
      defaultApplications = lib.genAttrs textMimes (_: "herdr-hx.desktop");
    };
  })
  ...
  (lib.mkIf pkgs.stdenv.isDarwin {
    ...
    # not /nix/store.
    home.activation.herdrHxApp = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      app="$HOME/Applications/herdr-hx.app"
      ...
      /usr/bin/osacompile -o "$app" -e '
        ...
        -- Launched without a file (e.g. `open -a herdr-hx`): do nothing,
      ...
      /usr/bin/plutil -replace CFBundleIdentifier -string io.mulatta.herdr-hx \
        "$app/Contents/Info.plist"
      /usr/bin/plutil -replace CFBundleDocumentTypes -json '${
        builtins.toJSON [
          {
            CFBundleTypeName = "herdr-hx documents";
    ...
    home.activation.herdrHxDuti = lib.hm.dag.entryAfter [ "herdrHxApp" ] ''
      set_herdr_hx_handler() {
        uti="$1"
        current="$(${pkgs.duti}/bin/duti -d "$uti" 2>/dev/null || true)"
        if [ "$current" = io.mulatta.herdr-hx ]; then
          return
        fi

        ${pkgs.duti}/bin/duti -s io.mulatta.herdr-hx "$uti" all 2>/dev/null || true
        current="$(${pkgs.duti}/bin/duti -d "$uti" 2>/dev/null || true)"
        if [ "$current" != io.mulatta.herdr-hx ]; then
          ${pkgs.duti}/bin/duti -s io.mulatta.herdr-hx "$uti" all
        fi
      }

      ${lib.concatMapStringsSep "\n" (uti: ''
        set_herdr_hx_handler ${lib.escapeShellArg uti}
      '') utis}
    '';
  })
]
